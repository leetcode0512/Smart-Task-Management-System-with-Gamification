/**
 * @file main.cpp
 * @brief Smart Task Management System with Gamification - Main Entry Point
 * 
 * 项目: 游戏化智能任务管理系统
 * 技术栈: C++17, SQLite3, ANSI控制台UI
 * 
 * 负责人: Mao Jingqi (成员E)
 * 团队成员:
 *   - 成员A (Fei Yifan): 提醒系统、成就系统
 *   - 成员B (Zhou Tianjian): 项目管理、热力图
 *   - 成员C (Kuang Wenqing): 任务管理
 *   - 成员D (Yu Zhixuan): 数据库层、DAO
 *   - 成员E (Mao Jingqi): 统计分析、UI、XP系统、主程序
 */

#include <iostream>
#include <string>
#include "database/DatabaseManager.h"
#include "ui/UIManager.h"
#include "statistics/StatisticsAnalyzer.h"
#include "gamification/XPSystem.h"

using namespace std;

/**
 * @brief 显示欢迎横幅
 */
void displayWelcomeBanner() {
    cout << "\n";
    cout << "\033[1;36m"; // 青色粗体
    cout << R"(
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
║   ███████╗███╗   ███╗ █████╗ ██████╗ ████████╗          ║
║   ██╔════╝████╗ ████║██╔══██╗██╔══██╗╚══██╔══╝          ║
║   ███████╗██╔████╔██║███████║██████╔╝   ██║             ║
║   ╚════██║██║╚██╔╝██║██╔══██║██╔══██╗   ██║             ║
║   ███████║██║ ╚═╝ ██║██║  ██║██║  ██║   ██║             ║
║   ╚══════╝╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝   ╚═╝             ║
║                                                           ║
║        Task Management System with Gamification          ║
║                    Version 1.0                            ║
║                                                           ║
╚═══════════════════════════════════════════════════════════╝
)" << "\033[0m"; // 重置颜色
    cout << "\n";
}

/**
 * @brief 初始化系统
 * @return 是否初始化成功
 */
bool initializeSystem() {
    cout << "\033[1;33m"; // 黄色粗体
    cout << "🚀 正在启动系统...\n" << "\033[0m";
    cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    
    // 1. 初始化数据库
    cout << "📦 [1/3] 初始化数据库..." << endl;
    DatabaseManager* db = DatabaseManager::getInstance();
    
    if (!db->initialize("task_manager.db")) {
        cerr << "\033[1;31m❌ 数据库初始化失败！\033[0m" << endl;
        cerr << "错误信息: " << db->getLastErrorMessage() << endl;
        return false;
    }
    
    cout << "\033[1;32m✅ 数据库初始化成功\033[0m" << endl;
    
    // 2. 验证数据库表
    cout << "🔍 [2/3] 验证数据库表..." << endl;
    
    bool allTablesExist = true;
    vector<string> requiredTables = {
        "tasks", "projects", "challenges", "reminders", 
        "achievements", "user_stats", "user_settings"
    };
    
    for (const string& table : requiredTables) {
        if (!db->tableExists(table)) {
            cerr << "\033[1;31m❌ 表 '" << table << "' 不存在\033[0m" << endl;
            allTablesExist = false;
        } else {
            cout << "\033[1;32m  ✓\033[0m 表 '" << table << "' 就绪" << endl;
        }
    }
    
    if (!allTablesExist) {
        cerr << "\033[1;31m❌ 部分数据库表缺失！\033[0m" << endl;
        return false;
    }
    
    cout << "\033[1;32m✅ 所有数据库表验证通过\033[0m" << endl;
    
    // 3. 检查数据库完整性
    cout << "🔒 [3/3] 检查数据库完整性..." << endl;
    
    if (!db->checkDatabaseIntegrity()) {
        cerr << "\033[1;31m❌ 数据库完整性检查失败！\033[0m" << endl;
        return false;
    }
    
    cout << "\033[1;32m✅ 数据库完整性检查通过\033[0m" << endl;
    
    cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    cout << "\033[1;32m🎉 系统初始化完成！\033[0m\n\n";
    
    return true;
}

/**
 * @brief 清理系统资源
 */
void cleanupSystem() {
    cout << "\n\033[1;33m🧹 正在清理系统资源...\033[0m\n";
    
    // 关闭数据库连接
    DatabaseManager::destroyInstance();
    
    cout << "\033[1;32m✅ 资源清理完成\033[0m\n";
}

/**
 * @brief 显示系统信息
 */
void displaySystemInfo() {
    cout << "\033[1;36m"; // 青色
    cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    cout << "📋 系统信息:\n";
    cout << "  • 项目名称: Smart Task Management System\n";
    cout << "  • 版本: 1.0.0\n";
    cout << "  • 技术栈: C++17, SQLite3\n";
    cout << "  • 开发团队: CSC3002 Group Project Team\n";
    cout << "  • 开发周期: 2025-11-08 ~ 2025-11-30\n";
    cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    cout << "\033[0m"; // 重置颜色
    cout << "\n";
}

/**
 * @brief 主函数
 */
int main(int argc, char* argv[]) {
    try {
        // 显示欢迎横幅
        displayWelcomeBanner();
        
        // 显示系统信息
        displaySystemInfo();
        
        // 初始化系统
        if (!initializeSystem()) {
            cerr << "\033[1;31m💥 系统初始化失败，程序退出\033[0m\n";
            return 1;
        }
        
        // 创建并运行UI管理器
        cout << "\033[1;36m🎮 启动用户界面...\033[0m\n\n";
        
        UIManager* ui = new UIManager();
        ui->run();
        
        // 清理UI资源
        delete ui;
        
        // 清理系统资源
        cleanupSystem();
        
        // 显示退出信息
        cout << "\n";
        cout << "\033[1;36m"; // 青色
        cout << "╔═══════════════════════════════════════════════════╗\n";
        cout << "║                                                   ║\n";
        cout << "║          👋 感谢使用！期待下次再见！              ║\n";
        cout << "║                                                   ║\n";
        cout << "╚═══════════════════════════════════════════════════╝\n";
        cout << "\033[0m"; // 重置颜色
        cout << "\n";
        
        return 0;
        
    } catch (const exception& e) {
        cerr << "\033[1;31m";
        cerr << "\n╔═══════════════════════════════════════════════════╗\n";
        cerr << "║                    💥 系统错误                    ║\n";
        cerr << "╚═══════════════════════════════════════════════════╝\n";
        cerr << "\n错误信息: " << e.what() << "\n";
        cerr << "\033[0m";
        
        // 尝试清理
        cleanupSystem();
        
        return 1;
    } catch (...) {
        cerr << "\033[1;31m";
        cerr << "\n╔═══════════════════════════════════════════════════╗\n";
        cerr << "║                  💥 未知错误                      ║\n";
        cerr << "╚═══════════════════════════════════════════════════╝\n";
        cerr << "\033[0m";
        
        // 尝试清理
        cleanupSystem();
        
        return 1;
    }
}
